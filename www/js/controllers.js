angular.module('starter.controllers', ['ngCordova'])
    ...
    .controller('MapCtrl', function ($scope, $ionicLoading, $compile, $state, $cordovaGeolocation) {
        function initialize() {
          ...
          var image = new google.maps.MarkerImage('/img/CarMarker2.png');
            ...
            $cordovaGeolocation.getCurrentPosition(options).then(function (position) {
              ...
              var image = new google.maps.MarkerImage('/img/CarMarker2.png');
                ...
                $scope.rows.forEach(function (item) {
                    ...
                    // Вешаем на маркер с АЗС ивент, вызывает по клику построение маршрута
                    // Почти полный копипаст контроллера для AZS_near
                    google.maps.event.addListener(marker_AZS, 'click', function(event) {
                        $scope.directionService = new google.maps.DirectionsService;
                        $scope.directionsDisplay = new google.maps.DirectionsRenderer;
                        ...
                        //В процессе построения маршрута – заного вносим на карту все АЗС, ибо карта обнуляется
                        $scope.rows.forEach(function (item) {
                            var myLatlng = new google.maps.LatLng(item.latitude, item.longitude);
